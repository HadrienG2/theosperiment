...
//Represents an item in a map of the memory, managed as a chained list at the moment.
...
struct MemMap {
  ...
  PIDs owners;
  MemMap* next_buddy; //This second chaining allows non-contiguous memory chunk
  MemMap* next_item;
  uint64_t padding;
  uint64_t padding2;
  MemMap() : location(0),
  ...
  MemMap* find_freechunk();
  MemMap* find_thischunk(addr_t location);
