 /*  Arch-agnostic part of physical memory management, ie managing pages of RAM
...
bool PhyMemManager::alloc_mapitems(PhyMemChunk* free_mem_override) {
    size_t remaining_freemem = 0, used_space;
    PhyMemChunk *allocated_chunk, *current_item, *free_chunk = NULL;
    ...
    current_item = (PhyMemChunk*) (allocated_chunk->location);
    for(used_space = sizeof(PhyMemChunk); used_space < allocated_chunk->size; used_space+= sizeof(PhyMemChunk)) {
        current_item = new(current_item) PhyMemChunk();
        current_item->next_mapitem = current_item+1;
        ++current_item;
    }
    current_item = new(current_item) PhyMemChunk();
    current_item->next_mapitem = free_mapitems;
    free_mapitems = (PhyMemChunk*) (allocated_chunk->location);
...
bool PhyMemManager::alloc_pids(PhyMemChunk* free_mem_override) {
    size_t remaining_freemem, used_space;
    PIDs *current_item;
    PhyMemChunk *allocated_chunk, *free_chunk = NULL;
...
void PhyMemManager::fill_mmap(const KernelInformation& kinfo) {
    size_t index;
    PhyMemChunk *current_item, *last_free = NULL, *previous_item;

    //Generate the first memory map item
    index = 0;
    phy_mmap = generate_chunk(kinfo, index);
    previous_item = phy_mmap;

    //If it is free, add it to the free memory pool
    if(phy_mmap->has_owner(PID_INVALID) && phy_mmap->allocatable) {
        if(last_free) {
            last_free->next_buddy = phy_mmap;
        } else {
            free_lowmem = phy_mmap;
        }
        last_free = phy_mmap;
    }
...
void PhyMemManager::fix_overlap(PhyMemChunk* first_chunk, PhyMemChunk* second_chunk) {
...
PhyMemChunk* PhyMemManager::generate_chunk(const KernelInformation& kinfo, size_t& index) {
    //Called during PhyMemManager initialization, this function generates a PhyMemChunk from a set
    ...
    //Allocate chunk
    PhyMemChunk* result = free_mapitems;
    free_mapitems = free_mapitems->next_mapitem;
...
void PhyMemManager::merge_with_next(PhyMemChunk* first_item) {
    PhyMemChunk* next_item = first_item->next_mapitem;
    ...
    next_item = new(next_item) PhyMemChunk();
...
bool PhyMemManager::split_chunk(PhyMemChunk* chunk, const size_t position) {
    ...
    PhyMemChunk* new_chunk = free_mapitems;
    free_mapitems = free_mapitems->next_mapitem;
