...
KernelMemoryMap* add_bios_mmap(KernelMemoryMap* kmmap_buffer, unsigned int *index_ptr, multiboot_info_t* mbd) {
  ...
  while(remaining_mmap > 0) {
    ...
    if((new_addr64>addr64+size64) && (addr64<0x100000)) {
      //A hole in the memory map provided by the BIOS and in low memory
      //almost certainly means memory-mapped I/O. We'll hence tag it as such
      kmmap_buffer[index].location = addr64+size64;
      kmmap_buffer[index].size = new_addr64-(addr64+size64);
      kmmap_buffer[index].nature = NATURE_RES;
      kmmap_buffer[index].name = (uint32_t) "Memory-mapped I/O";
      ++index;
